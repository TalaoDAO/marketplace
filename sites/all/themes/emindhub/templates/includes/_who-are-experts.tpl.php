<div class="title-wrapper">
    <div class="row">
        <div class="col-xs-12 bold paddingUD">
            <div class="dark-line">
                <div class="title upper width-18 width-xs-29 width-xxs-44"><?php print c_szWhoAreExperts; ?></div>
            </div>
        </div>
    </div>
</div>


<div class="row text-16">
    <div class="col-md-4">
        <div class="row">
            <div class="col-md-3 col-sm-2 col-xs-3">
                <?php print $atomiumImg; ?>
            </div>
            <div class="col-md-9 col-sm-10 col-xs-9">
                <span class="light-blue-text"><?php print c_szExperimentedPro; ?></span>
                <p><?php print c_szInYourNetwork; ?></p>
            </div>
        </div>
        <div class="little-wrapper">
            <div class="row">
                <div class="col-md-3 col-sm-2 col-xs-3">
                    <?php print $peopleImg; ?>
                </div>
                <div class="col-md-9 col-sm-10 col-xs-9">
                    <span class="light-blue-text"><?php print c_szWithEmindhub; ?></span>
                    <p><?php print c_szYouOrganize; ?></p>
                </div>
            </div>
        </div>
    </div>
    <div class="col-md-8 ondes-background">
        <div class="row">
            <div class="col-md-offset-2 col-xs-offset-2 col-xs-4 col-md-4 noPadding  center-text">
                <div class="row">
                    <div class="col-sm-4 col-xs-12 noPadding">
                        <span class="popup-hover">
                            <?php print $cercleImg; ?>
                            <span class="experts">
                                <?php print c_szWorldDb; ?>
                            </span>
                        </span>
                    </div>
                    <div class="col-sm-8 col-xs-12 paddingUD">
                        <?php print c_szAll; ?> <br><!--<span class="bold">--><?php print c_szAllExperts; ?><!--</span>-->
                    </div>
                </div>
            </div>
            <div class="col-md-4 col-xs-6 center-text">
                <div class="row">
                    <div class="col-sm-4 col-xs-12 noPadding">
                        <span class="popup-hover">
                        <?php print $cercleImg; ?>
                            <span class="sponsored">
                                <?php print c_szSponsoredByPeers; ?>
                            </span>
                        </span>
                    </div>
                    <div class="col-sm-8 col-xs-12 paddingUD">
                        <?php print c_szSponsored; ?> <br><!--<span class="bold">--><?php print c_szExperts; ?><!--</span>-->
                    </div>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-xs-4 center-text">
                <div class="row">
                    <div class="col-sm-4 col-xs-12 noPadding">
                        <span class="popup-hover">
                        <?php print $cercleImg; ?>
                            <span class="partners">
                                <?php print c_szYourPartnersAndEmployees; ?>
                            </span>
                        </span>
                    </div>
                    <div class="col-sm-8 paddingUD col-xs-12">
                        <?php print c_szYourNetwork; ?> <br><!--<span class="bold">--><?php print c_szActive; ?><!--</span>-->
                    </div>
                </div>
            </div>
            <div class="col-xs-4 center-text">
                <div class="row">
                    <div class="col-sm-4 col-xs-12 noPadding">
                        <span class="popup-hover">
                        <?php print $cercleImg; ?>
                            <span class="formers">
                                <?php print c_szOldsContributors; ?>
                            </span>
                        </span>
                    </div>
                    <div class="col-sm-8 col-xs-12 paddingUD">
                        <?php print c_szYours; ?> <br><!--<span class="bold">--><?php print c_szAlumnis; ?><!--</span>-->
                    </div>
                </div>
            </div>
            <div class="col-xs-4 center-text">
                <div class="row">
                    <div class="col-sm-4 col-xs-12 noPadding">
                        <span class="popup-hover">
                        <?php print $cercleImg; ?>
                            <span class="circles">
                                <?php print c_szOnDemandCircles; ?>
                            </span>
                        </span>
                    </div>
                    <div class="col-sm-8 col-xs-12 paddingUD">
                        <?php print c_szYouCircles; ?> <br><!--<span class="bold">--><?php print c_szPrivates; ?><!--</span>-->
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
