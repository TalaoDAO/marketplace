...
/**
 * Implements hook_og_features_registry_alter().
 * 
 ...
 * 
 * @param $entity
 *   The group entity that toggled features
 * @param $entity_id
 *   The entity id
 * @param $entity_type
 *   THe entity type
 ...
 *   inside the group
 */
function hook_og_features_toggle($entity, $entity_id, $entity_type, $features) {
  if (!$features['my_feature']) {
    db_query("DELETE FROM {og_things} WHERE etid = %d", $entity_id);
  }
}
 ...
 *   THe entity type
 */
function hook_og_features_disabled_alter(&$disabled, $entity, $entity_id, $entity_type) {
